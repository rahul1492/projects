...
class Contact extends CI_Controller {
	...
	public function index(){
		...
		{
			$sqlQry = 'SELECT * FROM contact_enquiry WHERE user_email like "'.$searchName.'%"';	
		}
		else
		{
			
			$sqlQry  = 'SELECT * FROM contact_enquiry' ;	
		}

		$dataCnt 		= 	$this->db->query($sqlQry);
		$data['total'] = $this->db->count_all('contact_enquiry');
	...
	public function multi_action(){
		if($_POST['action']=='delete'){
			foreach ($_POST['chk'] as $key) {
				$delete = $this->master_model->deleteRecord('contact_enquiry',array("id"=>$key));
			}
			if($delete==TRUE){
	...
	public function delete($id){
		if(isset($id)!=''){
			$id=base64_decode($id);
			$delete = $this->master_model->deleteRecord('contact_enquiry',array("id"=>$id));
			if($delete==TRUE){
	...
	public function viewenquiry($id){
		$id=base64_decode($id);
		$edit_record = $this->master_model->updateRecord('contact_enquiry',array('is_view'=>'1'),array("id"=>$id));
		$data['enquiry']=	$this->master_model->getRecords('contact_enquiry',array("id"=>$id));
